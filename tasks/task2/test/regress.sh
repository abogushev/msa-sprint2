...
PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${BOOKINGS_DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures-bookings-service.sql
...
#Измените тесты для booking сервиса так, чтобы они соотвестсвовали вашей текущей реализации.
BOOKINGS_URL="${BOOKINGS_URL:-localhost:9090}"
...
grpcurl -plaintext -d '' "${BOOKINGS_URL}" booking.BookingService/ListBookings  | grep -q 'test-user-2' && pass "Все бронирования получены" || fail "Бронирования не получены"
...
grpcurl -plaintext -d '{"user_id":"test-user-2"}' "${BOOKINGS_URL}" booking.BookingService/ListBookings | grep -q 'test-user-2' && pass "Бронирования test-user-2 найдены" || fail "Нет бронирований test-user-2"
...
grpcurl -plaintext -d '{"user_id":"test-user-3", "hotel_id":"test-hotel-1"}' "${BOOKINGS_URL}" booking.BookingService/CreateBooking | grep -q 'test-hotel-1' && pass "Бронирование прошло (без промо)" || fail "Бронирование (без промо) не прошло"
...
grpcurl -plaintext -d '{"user_id":"test-user-2", "hotel_id":"test-hotel-1", "promo_code":"TESTCODE1"}' "${BOOKINGS_URL}" booking.BookingService/CreateBooking | grep -q 'TESTCODE1' && pass "Бронирование с промо прошло" || fail "Бронирование с промо не прошло"
...
# 5. Ошибка — неактивный пользователь
response=$(grpcurl -plaintext -d '{"user_id":"test-user-0", "hotel_id":"test-hotel-1"}' \
   "${BOOKINGS_URL}" booking.BookingService/CreateBooking 2>&1 || true)
# Проверка результата
if [[ "$response" == *"Code: Internal"* ]]; then
...
response=$(grpcurl -plaintext -d '{"user_id":"test-user-2", "hotel_id":"test-hotel-3"}' \
   "${BOOKINGS_URL}" booking.BookingService/CreateBooking 2>&1 || true)
# Проверка результата
if [[ "$response" == *"Code: Internal"* ]]; then
    pass "Отклонено: недоверенный отель"
...
response=$(grpcurl -plaintext -d '{"user_id":"test-user-2", "hotel_id":"test-hotel-2"}' \
   "${BOOKINGS_URL}" booking.BookingService/CreateBooking 2>&1 || true)
# Проверка результата
if [[ "$response" == *"Code: Internal"* ]]; then
    pass "Отклонено: отель полностью забронирован"
